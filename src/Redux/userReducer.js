...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    edit_user: async (state, action) => {
      ...
      state.value = await axiosRequest.put(`/update/${id}`, {
        type,
        description,
        category,
      });
    ...
    getall: async (state ) => {   
      console.log("ninu") 
      let res= await axiosRequest.get(`/getAll`)
      state.value =res.data
    },
  },
});
export const { create_user, edit_user, delete_user,getall } = userSlice.actions;
